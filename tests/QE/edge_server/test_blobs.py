...
class TestBlobs(CBLTestClass):
    ...
    async def test_blobs_create_delete(
        ...
        assert "_attachments" in document, (
            "Error: '_attachments' field is missing in the document response"
        )
        assert f"blob_/{attachment_name}" in document["_attachments"], (
            f"Error: Attachment '{attachment_name}' not found in '_attachments'"
        )

        blob_metadata = document["_attachments"][f"blob_/{attachment_name}"]
        assert blob_metadata["content_type"] == "image/png", (
            f"Error: Expected content_type='image/png', but got '{blob_metadata['content_type']}'"
        )
        assert "digest" in blob_metadata, (
            "Error: 'digest' field is missing in blob metadata"
        )
        assert "length" in blob_metadata, (
            "Error: 'length' field is missing in blob metadata"
        )
        assert blob_metadata["length"] > 0, (
            f"Error: Blob length is invalid ({blob_metadata['length']})"
        )

        # Assert that the blob is also referenced in the document body
        assert attachment_name in document, (
            f"Error: '{attachment_name}' not found in document body"
        )
        blob_info = document[attachment_name]
        assert blob_info["@type"] == "blob", (
            f"Error: Expected '@type'='blob', but got '{blob_info.get('@type')}'"
        )
        assert blob_info["content_type"] == "image/png", (
            f"Error: Expected content_type='image/png', but got '{blob_info['content_type']}'"
        )
        assert blob_info["digest"] == blob_metadata["digest"], (
            f"Error: Blob digest mismatch, expected '{blob_metadata['digest']}', got '{blob_info['digest']}'"
        )
        assert blob_info["length"] == blob_metadata["length"], (
            f"Error: Blob length mismatch, expected '{blob_metadata['length']}', got '{blob_info['length']}'"
        )
        ...
        assert "_attachments" not in document, (
            "Error: '_attachments' field is present in the document response"
        )
    ...
    async def test_blob_get_nonexistent(
        ...
        try:
            ...
            assert CblEdgeServerBadResponseError, (
                "Able to retrieve nonexistent blob from document."
            )
    ...
    async def test_blob_delete_nonexistent(
        ...
        try:
            ...
            assert CblEdgeServerBadResponseError, (
                "Able to delete nonexistent blob from document."
            )
    ...
    async def test_blob_update_incorrect_rev(
        ...
        try:
            ...
            assert CblEdgeServerBadResponseError, (
                "Able to update blob with incorrect revision in document."
            )
    ...
    async def test_blob_put_nonexistent_doc(
        ...
        try:
            ...
            assert CblEdgeServerBadResponseError, (
                "Able to add blob to nonexistent document."
            )
    ...
    async def test_blob_exceeding_maxsize(
        ...
        try:
            ...
            assert CblEdgeServerBadResponseError, (
                "Able to add blob exceeding max size to document."
            )
    ...
    async def test_blob_special_characters(
        ...
        doc_id = "doc_2"
        logger.info(
            f"Adding a blob with special characters to document {doc_id} in Edge Server."
        )
