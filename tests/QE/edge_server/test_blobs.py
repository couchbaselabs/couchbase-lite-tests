...
class TestBlobs(CBLTestClass):
    ...
    async def test_blobs_create_delete(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        sg_config = {
            "bucket": "bucket-1",
            "scopes": {"_default": {"collections": {"_default": {"sync": "function(doc){channel(doc.channels);}"}}}},
        ...
        self.mark_test_step("Creating a database on Edge Server with replication to Sync Gateway; waiting for idle.")
        es_db_name = "db"
        ...
        assert blob_info["@type"] == "blob", f"Expected '@type'='blob', got '{blob_info.get('@type')}'"
        ...
        delete_resp = await edge_server.delete_sub_document(doc_id, rev_id, attachment_name, es_db_name)
    ...
    async def test_blob_update(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        self.mark_test_step("Verifying that Sync Gateway has 2 documents.")
        response = await sync_gateway.get_all_documents(sg_db_name, "_default", "_default")
        assert len(response.rows) == 2, f"Expected 2 documents on Sync Gateway, got {len(response.rows)}."
        self.mark_test_step("Verifying that Edge Server has 2 documents.")
        response = await edge_server.get_all_documents(es_db_name)
        assert len(response.rows) == 2, f"Expected 2 documents on Edge Server, got {len(response.rows)}."
