...
class TestEdgeServerChaos(CBLTestClass):
    ...
    async def test_edge_server_offline_sync_and_recovery(self, cblpytest, dataset_path) -> None:
        ...
        config_path = f"{file_path}/environment/edge_server/config/test_sgw_edge_server.json"
        ...
        # setup edge 2
        config_path2 = f"{file_path}/environment/edge_server/config/test_edge_to_edge_server.json"
        ...
        assert "error" not in status[0].keys(), f"Replication setup failure: {status[0]}"
        print(status)

        # get all documents from travel.hotels

        all_docs=await client.get_all_documents(db_name="travel",collection="travel.hotels")
        revmap=all_docs.revmap

        # delete all docs in collection travel.hotels in es3
        bulk_ops = [ BulkDocOperation({"_deleted": True}, _id=doc_id,rev=rev,optype="delete") for doc_id, rev in revmap.items()]
        ...
        edge1_docs = await edge_server1.get_all_documents( "travel", collection="travel.hotels")
        # Get from SGW
        sgw_docs = await sgw.get_all_documents("travel", scope="travel", collection="hotels")

        assert len(edge1_docs.rows) == len(sgw_docs.rows)== len(edge2_docs.rows)==0, \
        ...
        resp=await edge_server3.bulk_doc_op(
            bulk_ops,
            "travel",
            "travel",
            "hotels"
    ...
    async def perform_operation(self,client, optype, docs_dict,docgen,db_name,scope,collection,revmap):
        ...
        try:
            if optype == "create":
                ...
                doc_id=new_doc["_id"]
                response = await client.put_document_with_id(document=new_doc,db_name=db_name,scope=scope,collection=collection,doc_id=doc_id)
            ...
            if optype == "update":
                ...
                response = await client.put_document_with_id(doc_id=doc_id, document=updated_doc,db_name=db_name,scope=scope,collection=collection,rev=revmap[doc_id])
                docs_dict[doc_id] = updated_doc  # Update local copy
                revmap[doc_id] =response.get("rev")
                return response["ok"]

            if optype == "delete":
                response = await client.delete_document(doc_id,revid=revmap[doc_id],db_name=db_name,scope=scope,collection=collection)
                print(response)
                if response.get("ok"):
    ...
    async def test_edge_server_with_multiple_rest_clients(self, cblpytest, dataset_path) -> None:
        ...
        try:
            ...
            for i in range(1000):
                ...
                result=await self.perform_operation(client,op,docs_dict,docgen,db_name="db",scope="",collection="",revmap=revmap)
    ...
    async def kill_edge_perform_ops(self,kill_server:List,ops_server:List,docgen, revmap,docs_list):
        ...
        bulk_ops=[BulkDocOperation(doc, optype="update",rev=revmap.get(doc.get("_id"))) for doc in update_docs]
        ...
        for server in ops_server:
            all_docs = await server.get_all_documents(db_name="travel", collection="travel.hotels")
    ...
    #     start kill_servers
        ...
        for server in kill_server:
            for doc in update_docs:
                remote_doc = await server.get_document(db_name="travel", scope="travel", collection="hotels", doc_id=doc.get("_id"))
                for key, value in remote_doc.body.items():
                    assert doc.get(key) == value
    ...
    async def test_3_edge_with_sync(self, cblpytest, dataset_path) -> None:
        ...
        bulk_ops = [BulkDocOperation({"_deleted": True}, _id=doc_id, rev=rev, optype="delete") for doc_id, rev in
                    revmap.items()]
        ...
        edge1_docs = await edge_server1.get_all_documents("travel", collection="travel.hotels")
        edge2_docs = await edge_server2.get_all_documents( "travel", collection="travel.hotels")
        edge3_docs = await edge_server3.get_all_documents( "travel", collection="travel.hotels")

        assert len(edge1_docs.rows) == len(edge3_docs.rows) == len(edge2_docs.rows) == 0, \
        ...
        assert len(edge1_docs.rows) == len(edge3_docs.rows) == len(edge2_docs.rows) == 10000, \
            f"Collection hotels count mismatch in len(edge1_docs.rows): {len(edge1_docs.rows)} ,  len(edge3_docs.rows): {len(edge3_docs.rows)}, len(edge2_docs.rows):  {len(edge2_docs.rows)}"

        self.mark_test_step("killing edge_server1  ")
        docs_list=await self.kill_edge_perform_ops([edge_server1],[edge_server3,edge_server2],docgen,edge3_docs.revmap,docs_list)
        edge1_docs = await edge_server1.get_all_documents("travel", collection="travel.hotels")
        self.mark_test_step("killing edge_server2  ")
        docs_list=await self.kill_edge_perform_ops([edge_server2], [edge_server1, edge_server3], docgen,edge1_docs.revmap,docs_list)
        edge2_docs = await edge_server2.get_all_documents("travel", collection="travel.hotels")
        self.mark_test_step("killing edge_server3  ")
        docs_list=await self.kill_edge_perform_ops([edge_server3], [edge_server2, edge_server1], docgen,edge2_docs.revmap,docs_list)
        edge2_docs = await edge_server2.get_all_documents("travel", collection="travel.hotels")
        self.mark_test_step("killing edge_server1,edge_server3  ")
        docs_list=await self.kill_edge_perform_ops([edge_server1,edge_server3], [edge_server2], docgen,edge2_docs.revmap,docs_list)
        edge3_docs = await edge_server3.get_all_documents("travel", collection="travel.hotels")
        self.mark_test_step("killing edge_server1 ,edge_server2 ")
        docs_list=await self.kill_edge_perform_ops([edge_server1,edge_server2], [edge_server3], docgen,edge3_docs.revmap,docs_list)
        edge1_docs = await edge_server1.get_all_documents("travel", collection="travel.hotels")
        self.mark_test_step("killing edge_server2,edge_server3  ")
        docs_list=await self.kill_edge_perform_ops([edge_server2,edge_server3], [ edge_server1], docgen,edge1_docs.revmap,docs_list)
    ...
    async def test_kill_sgw_mid_replication(self, cblpytest, dataset_path) -> None:
        ...
        bulk_ops = [BulkDocOperation({"_deleted": True}, _id=doc_id, rev=rev, optype="delete") for doc_id, rev in
                    hotel_docs.revmap.items()]

        resp = await edge_server.bulk_doc_op(bulk_ops, "travel", "travel", "hotels")
        print(resp)

        self.mark_test_step("Verify document deleted")
        edge_docs = await edge_server.get_all_documents("travel", collection="travel.hotels")
        sgw_docs = await sgw.get_all_documents("travel", scope="travel", collection="hotels")
        ...
        update_docs= docgen.update_all_documents(docs_list)
        bulk_ops = [BulkDocOperation(doc, optype="update", rev=edge_docs.revmap.get(doc.get("_id"))) for doc in update_docs]
