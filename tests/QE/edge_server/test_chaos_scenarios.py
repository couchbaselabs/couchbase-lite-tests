...
class TestEdgeServerChaos(CBLTestClass):
    ...
    async def test_kill_sgw_mid_replication(self, cblpytest, dataset_path) -> None:
        ...
        await edge_server.bulk_doc_op(bulk_ops, db_name="travel", collection="travel.hotels")
        ...
        bulk_ops = [
            BulkDocOperation(body=doc, _id=id, optype="update", rev=edge_docs.revmap.get(id))
    ...
    async def test_3_edge_with_sync(self, cblpytest, dataset_path) -> None:
        ...
        bulk_ops = [
            BulkDocOperation({"_deleted": True}, _id=doc_id, rev=rev, optype="delete") for doc_id, rev in revmap.items()
        ]
        ...
        docs_list = docgen.generate_all_documents()
        bulk_ops = [BulkDocOperation(body=doc, _id=id, optype="create") for id, doc in docs_list.items()]
        await edge_server1.bulk_doc_op(bulk_ops, db_name="travel", collection="travel.hotels")
        ...
        assert len(edge1_docs.rows) == len(edge3_docs.rows) == len(edge2_docs.rows) == 1000, (
            f"Collection hotels count mismatch in len(edge1_docs.rows): {len(edge1_docs.rows)} ,  len(edge3_docs.rows): {len(edge3_docs.rows)}, len(edge2_docs.rows):  {len(edge2_docs.rows)}"
        )
    ...
    async def test_edge_server_offline_sync_and_recovery(self, cblpytest, dataset_path) -> None:
        ...
        all_docs = await edge_server3.get_all_documents(db_name="travel", collection="travel.hotels")
        revmap = all_docs.revmap
        bulk_ops = [
            BulkDocOperation({"_deleted": True}, _id=doc_id, rev=rev, optype="delete") for doc_id, rev in revmap.items()
        ]
    ...
    @pytest.mark.asyncio(loop_scope="session")
    async def test_edge_server_with_concurrent_rest_requests(self, cblpytest, dataset_path) -> None:
        ...
        # Seed database
        bulk_ops = [BulkDocOperation(body=doc, _id=doc_id, optype="create") for doc_id, doc in docs.items()]
        ...
        self.mark_test_step("Final consistency validation- actual document count vs expected document count")
        ...
        for row in final_docs.rows:
            assert docs.get(row.id), f"ID {row.id} exists on Edge server but not in local dict"
