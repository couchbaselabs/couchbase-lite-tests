...
class TestConflicts(CBLTestClass):
    ...
    async def test_conflicts(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        response = await sync_gateway.get_all_documents(sg_db_name, "_default", "_default")
        assert len(response.rows) == 15, f"Expected 15 documents, but got {len(response.rows)} documents."
        ...
        self.mark_test_step("Verify both services have 15 documents after conflict resolution")
        ...
        assert sg_doc_count == 15, f"Expected 15 documents in Sync Gateway after conflict resolution, but got {sg_doc_count} documents."
        logger.info(f"Sync Gateway has {sg_doc_count} documents after conflict resolution.")
        
        # Check Edge Server documents
        es_response = await edge_server.get_all_documents(es_db_name)
        es_doc_count = len(es_response.rows)
        assert es_doc_count == 15, f"Expected 15 documents in Edge Server after conflict resolution, but got {es_doc_count} documents."
        logger.info(f"Edge Server has {es_doc_count} documents after conflict resolution.")

        logger.info("Both Sync Gateway and Edge Server have 15 documents after conflict resolution")
        
        self.mark_test_step("Compare document contents between Sync Gateway and Edge Server")
        ...
        assert sg_ids == es_ids, f"Document ID mismatch: Sync Gateway has {sg_ids - es_ids} extra IDs, Edge Server has {es_ids - sg_ids} extra IDs"
        ...
        for doc_id in sg_ids:
            ...
            sg_doc = await sync_gateway.get_document(sg_db_name, doc_id, "_default", "_default")
            ...
            if doc_id in conflict_doc_ids:
                ...
                if sg_doc.body.get("edge_server") == es_doc.body.get("edge_server"):
                    ...
                    logger.info(f"Conflict resolved for {doc_id}: both services have consistent content")
                else:
                    logger.warning(f"Conflict not fully resolved for {doc_id}: SG={sg_doc.body.get('edge_server')}, ES={es_doc.body.get('edge_server')}")
            else:
                # Documents 1-10 should be identical
                assert sg_doc.body == es_doc.body, f"Document {doc_id} content mismatch between services"
                logger.info(f"Document {doc_id} content matches between services")
        
        logger.info(f"Conflict resolution summary: {conflicts_resolved}/5 conflicts resolved")
        
        # Verify that at least some conflicts were resolved
        assert conflicts_resolved > 0, "No conflicts were resolved during the test"
        
        logger.info("Both Sync Gateway and Edge Server have 15 documents with resolved conflicts")
