...
class TestQueryEdgeServer(CBLTestClass):
    ...
    async def test_named_queries(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        # Execute named query
        response = await client.named_query(db_name="names",name="user_by_email",
            params={"email": ["jewel.macartney@nosql-matters.org","macartney@nosql-matters.org"]}
        )
    ...
    async def test_adhoc_queries(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        expected_results = {"first":"Dorthey","last":"Gracy","gender":"female","birthday":"1967-08-25","email":["dorthey.gracy@nosql-matters.org","gracy@nosql-matters.org"],"phone":["740-8360831"],"city":"Waldo","state":"OH","likes":["snowboarding","driving"]}

        self.mark_test_step("Executing ad-hoc query")
        response = await client.adhoc_query(db_name="names",query=query['query'],params=query['params'])
        self.mark_test_step(f"Query result: {response}")
        assert DeepDiff(expected_results,response[0]) == {}


    @pytest.mark.asyncio
    async def test_negative_scenarios(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
