...
class TestEdgeToEdgeMutualTLS(CBLTestClass):
    ...
    async def _install_peer_credentials(target: EdgeServer) -> None:
        """
        Copies the runner's CA and client certificate onto the target host.

        ``~/.cbl_certs`` is rewritten by each host's provisioning pass, so it holds the
        credentials of whichever Edge Server was set up last. The source must be a host
        whose CA signed that client certificate -- with the current per-host CA scheme
        that means the last-provisioned one.
        """
    ...
    async def test_edge_to_edge_replication_with_mtls(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        self.mark_test_step("Wait for the replication to become idle an no errors")
