...
class TestLogging(CBLTestClass):
    ...
    async def test_audit_logging_default(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        log = await edge_server.check_log("57345")
        print(log)
        ...
        log = await edge_server.check_log("57346")
        ...
        log = await edge_server.check_log("57356")
        assert len(log) > 0, "Audit log for stop of replication event not found"
        print(log)
        ...
        # Create
        log = await edge_server.check_log("57358")
        assert log == [], "Audit log for create event found"
        # Read
        log = await edge_server.check_log("57359")
        assert log == [], "Audit log for read event found"
        # Update
        log = await edge_server.check_log("57360")
        assert log == [], "Audit log for update event found"
        # Delete
        log = await edge_server.check_log("57361")
    ...
    async def test_audit_logging_enabled(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        assert len(log) > 0, "Audit log for start of replication event not found"
        log = await edge_server.check_log("57356")
        assert len(log) > 0, "Audit log for stop of replication event found"
        logger.info("Audit logs checked for replication events")
        ...
        doc = {"id": doc_id, "channels": ["public"], "timestamp": datetime.utcnow().isoformat()}
