...
class TestSystem(CBLTestClass):
        ...
        while datetime.now() < end_time:
            ...
            if cycle == "sync_gateway":
                ...
                if "update" in operations:
                    ...
                    response = await sync_gateway.update_document(
                        sg_db_name, doc_id, updated_doc, rev_id
                    )
                    assert response is not None, (
    ...
    @pytest.mark.asyncio(loop_scope="session")
    async def test_system_multiple_clients_l(
        self, cblpytest: CBLPyTest, dataset_path: Path
    ) -> None:
        self.mark_test_step(
        ...
        factory = ClientFactory(
            ...
            num_clients_per_vm=int(num_of_clients / len(http_clients)),
        )
        # print(factory.vms)
        ...
        while datetime.now() < end_time:
            ...
            self.mark_test_step(
                f"Client {client} selected, continuing with the test..."
            )
            ...
            for i in range(1, num_of_clients + 1):
                methods[i] = {
                    "method": "get_document",
                    "params": {"db_name": es_db_name, "doc_id": last_three_docs[-2]},
                }
            # print(methods)
            ...
            if cycle == "sync_gateway":
                ...
                if "update" in operations:
                    ...
                    updated_docs = []
                    all_docs = await sync_gateway.get_all_documents(
                        sg_db_name, "_default", "_default"
                    )
                    ...
                    for doc in all_docs.rows[-5:]:
                        ...
                        updates.append(
                            DocumentUpdateEntry(doc.id, doc.revid, updated_doc)
                        )
                    ...
                    for i in updated_docs:
                        ...
                        assert document is not None, (
                            f"Document {i} does not exist on the edge server"
                        )
                ...
                if "delete" in operations:
                    ...
                    (
                        ...
                        error,
                        failed,
                    ) = await factory.make_unique_params_client_request(methods)
                    if len(error) > 0:
                        print(error)

                    assert response[client].get("ok") is True, (
                        f"Failed to delete document {doc_id} via Edge Server"
                    )
        ...
        while datetime.now() < end_time:
            ...
            if cycle == "sync_gateway":
                ...
                if "update" in operations:
                    ...
                    if not edge_server_down:
                        logger.info(
                            f"Validating update for {updated_docs} on Edge Server"
                        )
