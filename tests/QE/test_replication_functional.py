...
class TestReplicationFunctional(CBLTestClass):
    ...
    async def test_roles_replication(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        await sync_gateway.add_role("posts", "role1", {"_default": {"posts": {"admin_channels": ["group1"]}}})

        self.mark_test_step("Create role2 with access to group2 channel.")
        await sync_gateway.add_role("posts", "role2", {"_default": {"posts": {"admin_channels": ["group2"]}}})
    ...
    async def test_CBL_SG_replication_with_rev_messages(
        ...
        await sync_gateway.purge_document("doc_1", "short_expiry", "_default", "_default")
    ...
    async def test_replication_behavior_with_channelRole_modification(
        ...
        await sync_gateway.add_role("posts", "testrole", {"_default": {"posts": {"admin_channels": ["group1"]}}})
        ...
        await sync_gateway.add_role("posts", "testrole", {"_default": {"posts": {"admin_channels": ["group2"]}}})
    ...
    @pytest.mark.asyncio(loop_scope="session")
    async def test_default_conflict_withConflicts_withChannels(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
