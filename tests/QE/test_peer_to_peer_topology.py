...
class TestPeerToPeerTopology(CBLTestClass):
    ...
    async def test_peer_to_peer_topology_mesh(self, cblpytest: CBLPyTest, num_of_docs, continuous, replicator_type):
        ...
        for phase in range(1, 4):
            ...
            for target_idx in target_peers:
                listener = Listener(all_dbs[target_idx], ["_default._default"], 59840, disable_tls=True)
    ...
    async def test_peer_to_peer_topology_loop(self, cblpytest: CBLPyTest, num_of_docs, continuous, replicator_type):
        ...
        for phase in range(1, 4):
            ...
            listener = Listener(all_dbs[target_peer_idx], ["_default._default"], 59840, disable_tls=True)
            await listener.start()
            ...
            self.mark_test_step(f"Start replication from peer {phase} to peer {target_peer_idx + 1}")
            await replicator.start()

            self.mark_test_step(f"Wait for replication from peer {phase} to peer {target_peer_idx + 1} to complete")
            target_activity = ReplicatorActivityLevel.IDLE if continuous else ReplicatorActivityLevel.STOPPED
            status = await replicator.wait_for(
                target_activity,
                timeout=timedelta(seconds=300),
