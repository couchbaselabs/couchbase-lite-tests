...
class TestSgwUpgrade(CBLTestClass):
    ...
    async def test_replication_and_persistence_after_upgrade(
        ...
        docs_before = await sg.get_all_documents(sg_db, scope, collection, True)
        revs_before = {row.id: row.revid for row in docs_before.rows}
        print(f"Documents in SGW before replication: {revs_before}")

        self.mark_test_step("Update docs on SGW")
        updated_sg_docs: list[DocumentUpdateEntry] = []
        for row in docs_before.rows:
            if row is None or row.doc is None:
                continue
            updated_sg_docs.append(
                DocumentUpdateEntry(
                    row.id,
                    revs_before[row.id],
                    body={
        ...
        await sg.update_documents(sg_db, updated_sg_docs)
        ...
        for doc_id in revs_before:
            ...
            assert after_num > before_num, (
                f"Doc {doc_id}: Revision didn't progress on SGW. "
                f"Before={before_rev} (rev_num={before_num}), After={after_rev} (rev_num={after_num})"
            )
