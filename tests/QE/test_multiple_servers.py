...
async def _setup_database_and_user(
    ...
    async with sg.create_user_client(sg_db, user_name, user_password, channels) as client:
        yield client
    ...
    async def test_rebalance_sanity(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        async with _setup_database_and_user(
            ...
            if supports_version_vectors:
                ...
                original_vvs = {
                    entry.id: entry.changes[0] if entry.changes else None for entry in changes_initial.results
                }
