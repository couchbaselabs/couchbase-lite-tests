...
class TestReplicationAutoPurge(CBLTestClass):
    ...
    async def test_remove_docs_from_channel_with_auto_purge_enabled(
        ...
        for doc in lite_all_docs["_default.posts"]:
            if doc.id == "post_1":
                ...
                await cloud.sync_gateway.delete_document("post_4", doc.rev, "posts", collection="posts")
        ...
        for update in replicator.document_updates:
            if update.document_id == "post_1":
                assert update.flags & ReplicatorDocumentFlags.ACCESS_REMOVED, "Access removed flag missing from post_1"
    ...
    async def test_revoke_access_with_auto_purge_enabled(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        await cloud.sync_gateway.add_user("posts", "user1", "pass", collection_access_dict)
    ...
    async def test_filter_removed_access_documents(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        await cloud.sync_gateway.update_documents("posts", channel_access_dict, "_default", "posts")
        ...
        repl2 = Replicator(
            ...
            collections=[
                ReplicatorCollectionEntry(
                    ["_default.posts"],
                    pull_filter=ReplicatorFilter("documentIDs", {"documentIDs": {"_default.posts": ["post_1"]}}),
                )
            ],
            replicator_type=ReplicatorType.PULL,
            continuous=True,
    ...
    async def test_remove_user_from_role(
        ...
        self.mark_test_step("Update user by removing `group2` from the `admin_channels` property.")
        ...
        """)
        assert len(replicator.document_updates) == 2, "Incorrect document replication count"
        for update in replicator.document_updates:
            assert update.document_id == "post_4" or update.document_id == "post_5", (
    ...
    async def test_remove_role_from_channel(self, cblpytest: CBLPyTest, dataset_path: Path, auto_purge_enabled: bool):
        ...
        self.mark_test_step("Add a role on Sync Gateway `role1` with access to channel `group3` in `_default.posts`")
        await cloud.sync_gateway.add_role("posts", "role1", {"_default": {"posts": {"admin_channels": ["group3"]}}})
        ...
        lite_all_docs = await db.get_all_documents("_default.posts")
        n_docs = len(lite_all_docs["_default.posts"])
        assert n_docs == 6, f"Incorrect number of initial documents replicated (expected 5; got {n_docs}"
        expected_docs = {"post_1", "post_2", "post_3", "post_4", "post_5", "post_6"}
        for doc in lite_all_docs["_default.posts"]:
            assert doc.id in expected_docs, f"Unexpected document found after initial replication: {doc.id}"

        self.mark_test_step("Update `role1` by removing `group3` from the `admin_channels` property.")
        await cloud.sync_gateway.add_role("posts", "role1", {"_default": {"posts": {"collection_access": {}}}})
        ...
        """)
        assert len(replicator.document_updates) == 1, "Incorrect document replication count"

        for update in replicator.document_updates:
            assert update.document_id == "post_6", f"Unexpected document update found -> '{update.document_id}'"
    ...
    async def test_pull_after_restore_access(self, cblpytest: CBLPyTest, dataset_path: Path):
        ...
        """)
        remote_doc = await cloud.sync_gateway.get_document("posts", "post_1", collection="posts")
        ...
        self.mark_test_step("Wait for a document event regarding `post_1` with no error")
        repl.clear_document_updates()
        await repl.wait_for_all_doc_events(
            {WaitForDocumentEventEntry("_default.posts", "post_1", ReplicatorType.PULL, None)}
        )
    ...
    async def test_push_after_remove_access(self, cblpytest: CBLPyTest, dataset_path: Path):
        ...
        self.mark_test_step("Wait for a document event regarding `post_1` with no error")
        await repl.wait_for_all_doc_events(
            {WaitForDocumentEventEntry("_default.posts", "post_1", ReplicatorType.PUSH, None)}
        )
    ...
    async def test_auto_purge_after_resurrection(self, cblpytest: CBLPyTest, dataset_path: Path, remove_type: str):
        ...
        self.mark_test_step(f"Perform a {remove_type} operation to remove post_1 on SGW")
        if remove_type == "delete":
            await cloud.sync_gateway.delete_document("post_1", rev_ids["post_1"], "posts", collection="posts")
        else:
            await cloud.sync_gateway.purge_document("post_1", "posts", collection="posts")
