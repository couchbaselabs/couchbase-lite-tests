...
class TestReplicationAutoPurge(CBLTestClass):
    ...
    async def test_remove_docs_from_channel_with_auto_purge_enabled(
        ...
        for doc in lite_all_docs["_default.posts"]:
            if doc.id == "post_1":
                ...
                await sync_gateway.delete_document("post_4", doc.rev, "posts", collection="posts")
    ...
    async def test_filter_removed_access_documents(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        await sync_gateway.update_documents("posts", channel_access_dict, "_default", "posts")
    ...
    async def test_remove_role_from_channel(
        ...
        await sync_gateway.add_role("posts", "role1", {"_default": {"posts": {"admin_channels": ["group3"]}}})
        ...
        await sync_gateway.add_role("posts", "role1", {"_default": {"posts": {"collection_access": {}}}})
    ...
    async def test_auto_purge_after_resurrection(
        ...
        if remove_type == "delete":
            await sync_gateway.delete_document("post_1", rev_ids["post_1"], "posts", collection="posts")
        else:
