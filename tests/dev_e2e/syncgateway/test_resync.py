...
class TestSyncGatewayResync(CBLTestClass):
    ...
    async def _load_documents(
        ...
        """Writes documents to scope1.col1, batched to keep requests under the client timeout."""
    ...
    async def test_resync_simple(self, cblpytest: CBLPyTest, *, change_sync_function: bool) -> None:
        ...
        await sg_cluster.round_robin_node.start_resync(db_name, reset=True)

        self.mark_test_step(f"Wait until the resync operation on database '{db_name}' completes.")
        final_status = await sg_cluster.round_robin_node.wait_for_resync_completed(db_name)
    ...
    async def test_resync_stop_resume(self, cblpytest: CBLPyTest) -> None:
        ...
        await sg_cluster.round_robin_node.stop_resync(db_name)

        self.mark_test_step(
            f"Check that the resync status is 'stopped' for database '{db_name}', "
            f"and that it stopped before processing every document."
        )
        stopped_status = await sg_cluster.round_robin_node.wait_for_resync_stopped(db_name)
        ...
        await sg_cluster.round_robin_node.start_resync(db_name)
