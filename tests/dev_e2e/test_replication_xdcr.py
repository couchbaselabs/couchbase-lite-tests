...
class TestReplicationXdcr(CBLTestClass):
    async def setup_xdcr_clusters(
        ...
        """
        self.mark_test_step("Stop xdcr from cluster 1 to cluster 2 if it's on.")
        cblpytest.couchbase_servers[0].stop_xcdr(
            cblpytest.couchbase_servers[1], dataset_name
        )

        self.mark_test_step("Stop xdcr from cluster 2 to cluster 1 if it's on.")
        cblpytest.couchbase_servers[1].stop_xcdr(
            cblpytest.couchbase_servers[0], dataset_name
        )

        self.mark_test_step("Reset SG in cluster 1 and load dataset.")
        cloud1 = CouchbaseCloud(
            cblpytest.sync_gateways[0], cblpytest.couchbase_servers[0]
        )
        await cloud1.configure_dataset(dataset_path, dataset_name)

        self.mark_test_step("Reset SG in cluster 2 and load dataset.")
        ...
        self.mark_test_step("Start xdcr from cluster 1 to cluster 2.")
        cblpytest.couchbase_servers[0].start_xdcr(
            cblpytest.couchbase_servers[1], dataset_name
        )

        self.mark_test_step("Start xdcr from cluster 2 to cluster 1.")
        cblpytest.couchbase_servers[1].start_xdcr(
            cblpytest.couchbase_servers[0], dataset_name
        )

        self.mark_test_step("Wait 5 secs to ensure that cluster are ready.")
        time.sleep(5);
    ...
    async def test_push_and_pull_with_xdcr(self, cblpytest: CBLPyTest, dataset_path: Path):
        '''
        Summary:
            1. Push and pull 100 names docs from SG1 via a load balancer.
            ...
            5. Add, update, and delete docs from SG2.
        ...
        await compare_local_and_remote(
            ...
            bucket="names",
            collections=["_default._default"],
            skipCompareRevs=cblpytest.config.dataset_version_at(0) == "3.2"
        )
    ...
    async def test_fail_over(self, cblpytest: CBLPyTest, dataset_path: Path):
        ...
        Summary:
            ...
            7. Add 1 doc (name_201), update 1 doc (name_1), and delete 1 docs (name_2) in the local database.
            8. Add 1 doc (name_301), update 1 doc (name_101), and delete 1 docs (name_102) in SG2.
        ...
        self.mark_test_step("Start the replicator with header X-Backend=sg-1 to switch to SG2.")
        replicator = Replicator(
            db,
            repl_url,
            replicator_type=ReplicatorType.PUSH_AND_PULL,
