...
class TestMultipeer(CBLTestClass):
    ...
    @pytest.mark.asyncio(loop_scope="session")
    @pytest.mark.parametrize("mode", [RapidChangesMode.START, RapidChangesMode.STOP])
    @pytest.mark.skip("CBL-7100")
    async def test_rapid_availability_changes(
        self, cblpytest: CBLPyTest, mode: RapidChangesMode
    ):
        self.mark_test_step(
            "Reset local database and load `empty` dataset on two devices"
        )
        reset_tasks = [
            ts.create_and_reset_db(["db1"]) for ts in cblpytest.test_servers[:2]
        ]
        all_devices_dbs = await asyncio.gather(*reset_tasks)
        all_dbs = [dbs[0] for dbs in all_devices_dbs]

        self.mark_test_step("""
            Start a multipeer replicator on each device
                * peerGroupID: “com.couchbase.testing”
                * identity: anonymous
                * authenticator: accept-all (null)
                * collections: default collection                 
        ...
        for _ in range(5):
            self.mark_test_step("Stop multipeer replicator 2 after 5 seconds")
            await asyncio.sleep(5)
            await multipeer_replicators[1].stop()

            self.mark_test_step("Start multipeer replicator 2 after 5 seconds")
            await asyncio.sleep(5)
            await multipeer_replicators[1].start()

        if mode == RapidChangesMode.STOP:
            self.mark_test_step("Stop multipeer replicator 2 after 5 seconds")
            await asyncio.sleep(5)
            await multipeer_replicators[1].stop()

            self.mark_test_step("Verify that peer 2 is not visible to peer 1")
            status = await multipeer_replicators[0].get_status()
            assert len(status.replicators) == 0, (
                "Peer 2 should not be visible to peer 1"
            )
        else:
            self.mark_test_step("Verify that peer 2 is visible to peer 1")
            status = await multipeer_replicators[0].get_status()
            assert len(status.replicators) == 1, "Peer 2 should be visible to peer 1"
