...
class TestReplicationFilter(CBLTestClass):
    ...
    async def test_push_document_ids_filter(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        ], authenticator=ReplicatorBasicAuthenticator("user1", "pass"), enable_document_listener=True, pinned_server_cert=cblpytest.sync_gateways[0].tls_cert())
        ...
        self.mark_test_step("Check that only docs specified in the documentIDs filters are replicated except `travel.airline`.`airline_1000`")
