...
class TestCrud(CBLTestClass):
    ...
    async def test_single_doc_crud(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        self.mark_test_step("fetch previous revid single doc")
        # doc_prev = await edge_server.get_document(db_name,doc_id=id2, revid=rev2)
        # self.mark_test_step(f"fetched previous rev single doc: {doc_prev}")
        # assert doc_prev.body.get("test") == doc.body.get("test")
    ...
    async def test_multiple_doc_crud(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        created=[]
        for i in range(1,11):
            bulk_changes.append(BulkDocOperation(_id=f'name_{i+200}',body={"rev":1, "idx":i})) #insertions
            created.append(f'name_{i+200}')
            bulk_changes.append(BulkDocOperation(_id=all_docs.rows[i].id,body={},rev=all_docs.rows[i].revid,optype="delete")) #deletions
            deleted.append(all_docs.rows[i].id)
            bulk_changes.append(
                BulkDocOperation(_id=all_docs.rows[i+10].id, body={"rev": 2, "idx": i+10}, rev=all_docs.rows[i+10].revid,
                                 optype="update"))
        ...
        client_request_dict={
            1:{"method":"get_all_documents","params":{"db_name":db_name,"keys":created}},
            2:{"method":"get_all_documents","params":{"db_name":db_name,"keys":deleted}},
            3:{"method":"get_all_documents","params":{"db_name":db_name,"keys":updated}},
        }
    ...
    async def test_sub_doc_crud(self, cblpytest: CBLPyTest, dataset_path: Path) -> None:
        ...
        resp = await edge_server.put_sub_document(id=id1, revid=rev1,key="test_key",db_name=db_name,value=sample_value)
        ...
        resp=await edge_server.delete_sub_document(id1,resp.get("rev"),"test_key",db_name)
