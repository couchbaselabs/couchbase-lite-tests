...
class TestEdgeServerSync(CBLTestClass):
    ...
    async def test_edge_to_sgw_replication(self, cblpytest: CBLPyTest, dataset_path: Path):
        ...
        for collection in ["travel.airlines", "travel.airports", "travel.hotels", "travel.landmarks", "travel.routes"]:
            # Get from Edge
            edge_docs = await client.get_all_documents(
                "travel",
        ...
        sgw_doc = await sgw.get_document(db_name="travel",scope="travel", collection="airlines",doc_id= "airline_10000")
    ...
    async def test_edge_to_edge_replication(self, cblpytest: CBLPyTest, dataset_path: Path):
        ...
        # Verify update propagated to SGW
        sgw_doc = await edge_server2.get_document(db_name="travel", scope="travel", collection="airlines",
