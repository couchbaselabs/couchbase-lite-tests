...
IF "%status%"=="install" (
  ...
  echo %service_name%.exe //IS//%service_name% --Install=%location%\\%service_name%.exe --Description=%service_name% --Jvm=auto --Classpath=%location%\\%jar_name% --StartMode=jvm --StartClass=com.couchbase.lite.javadesktop.mobiletest.TestServerApp --StartMethod=windowsService --StartParams=start --StopMode=jvm --StopClass=com.couchbase.lite.javadesktop.mobiletest.TestServerApp --StopMethod=windowsService --StopParams=stop --LogPath=%location%\\logs --StdOutput=auto --StdError=auto --ServiceUser=.\%service_user% --ServicePassword=%service_pwd%
