...
extension Handlers {
    static let startListener: EndpointHandler<ContentTypes.Listener> = { req throws in
        ...
        let id = try dbManager.startListener(dbName: listenerStartRq.database, collections: listenerStartRq.collections, port: listenerStartRq.port, disableTLS: disableTLS,identity: listenerStartRq.identity)
