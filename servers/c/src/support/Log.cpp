...
static void log(CBLLogLevel level, CBLLogDomain domain, const char *format, ...) {
    va_list args;
...
void ts_support::logger::init(LogLevel level) {
    sLogLevel = level;
    CBLLog_SetCallbackLevel((CBLLogLevel) level);
    CBLLog_SetCallback([](CBLLogDomain domain, CBLLogLevel level, FLString msg) {
        log(level, domain, "%.*s", msg.size, msg.buf);
    });
}

void ts_support::logger::log(LogLevel level, const char *format, ...) {
    if (level >= sLogLevel) {
        va_list args;
        va_start(args, format);
