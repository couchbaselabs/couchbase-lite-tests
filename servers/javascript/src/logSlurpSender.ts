...
export class LogSlurpSender extends WebSocketClient implements Disposable {
    constructor(
        ...
        if (typeof url === "string") url = new URL(`ws://${url}/openLogStream`);
    ...
    async waitForConnected(timeoutMs?: number): Promise<void> {
        ...
        if (this.state === WebSocketState.Closed) throw new Error(this.error ?? "WebSocket already closed");
        return new Promise<void>((resolve, reject) => {
            ...
            this.onStateChange = (state) => {
                prev?.(state);
                if (state === WebSocketState.Open) finish(resolve);
            ...
            if (timeoutMs)
                timer = setTimeout(
                    () => finish(() => reject(new Error("Timeout waiting for WebSocket open"))),
                    timeoutMs,
                );
        });
    }
    ...
    stopLogging() {
        ...
        for (let logger of config.loggers) {
            ...
            if (i !== undefined && i >= 0) logger.sinks?.splice(i, 1);
        }
    ...
    private sendLog(record: logtape.LogRecord) {
        ...
        if (category[0] === LogCategory) category = category.slice(1);
        const message = `[${category.join(".")}] ${record.message.join("")}`;

        if (this.isOpen) this.send(message);
        else if (this.#buffer) this.#buffer.push(message);
    }

    protected override onOpen() {
        for (const message of this.#buffer!) this.send(message);
