...
export class TestServer extends WebSocketClient {
    ...
    protected override onTextMessage(message: string) {
        ...
        if (this.delegate) {
            ...
            if (handler) handler = handler.bind(this.delegate); // sets the 'this' argument
        }

        if (!handler) handler = this.#handlers.get(command);
    ...
    private sendResponse(request: TestRequest, result?: object | void, error?: Error) {
        if (error) this.logger.warn`Sending response #${request.ts_id} with error: ${error}`;
        else this.logger.info`Sending response #${request.ts_id}`;
