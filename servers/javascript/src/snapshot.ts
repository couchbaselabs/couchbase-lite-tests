...
export class Snapshot {
    ...
    async verify(
        ...
        for (let [collection, id, oldDoc] of this.#documents) {
            ...
            if (oldDoc || update) {
                ...
                if (oldDoc) {
                    if (update) expected = await this.#applyUpdate(oldDoc, update, blobLoader);
                    else expected = oldDoc;
                } else {
    ...
    async #applyUpdate(
        ...
        blobLoader: BlobLoader,
    ): Promise<cbl.CBLDocument | undefined> {
        if (update.type !== "UPDATE") return undefined;
        ...
        if (update.updatedProperties !== undefined) {
            for (const updates of update.updatedProperties) {
                for (const key of Object.getOwnPropertyNames(updates)) patch(key, updates[key]);
            }
        }
        if (update.removedProperties !== undefined) {
            for (const key of update.removedProperties) patch(key, undefined);
        }
    ...
    #compareDocs(
        expected: cbl.CBLDocument | undefined,
        ...
        const compare = (exp?: cbl.JSONValue, act?: cbl.JSONValue): boolean => {
            ...
            if (typeof exp !== "object" || exp === null) {
                ...
                if (!Array.isArray(act)) return fail();
                // Compare arrays:
                const expLen = Math.max(exp.length, act.length);
                for (let i = 0; i < expLen; ++i) {
                    path.push(i);
                    if (!compare(exp[i], act[i])) return false;
                    path.pop();
                }
                return true;
            } else {
                ...
                for (const key of expKeys) {
                    path.push(key);
                    if (!compare(exp[key], act[key])) return false;
                    path.pop();
                }
        ...
        if (expected) {
            ...
            if (actual) {
                ...
                if (updateType === "DELETE") response.description = "was not deleted";
                else if (updateType === "PURGE") response.description = "was not purged";
            }
        }
...
class DocumentMap<T> {
    ...
    *[Symbol.iterator](): Generator<[string, cbl.DocID, T]> {
        for (const [collection, docs] of this.#map) {
            for (const [id, doc] of docs) yield [collection, id, doc];
        }
    }
