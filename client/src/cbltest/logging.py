...
class LogSlurpHandler(Handler):
    def __init__(self, url: str, id: str):
        ...
        self.__url = url
        self.__id = id
        self.__ws = create_connection(f"ws://{url}/openLogStream", header=[f"CBL-Log-ID: {id}", "CBL-Log-Tag: test-client"])
    ...
    def close(self):
        ...
        resp = s.get(f"http://{self.__url}/retrieveLog", headers={"CBL-Log-ID": self.__id}, stream=True)
