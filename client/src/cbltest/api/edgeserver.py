...
class EdgeServer:
    ...
    async def get_document(
        ...
        with self.__tracer.start_as_current_span(
            ...
            response = await self._send_request(
                "get", f"/{keyspace}/{doc_id}", curl=curl
            )
            if curl:
                return response
    ...
    async def get_db_info(
        self, db_name: str, scope: str = "", collection: str = "", curl: bool = False
    ):
        with self.__tracer.start_as_current_span(
            "get database info",
            attributes={
    ...
    async def start_replication(
        ...
        with self.__tracer.start_as_current_span(
            ...
            response = await self._send_request(
                "post", "/_replicate", JSONDictionary(payload), curl=curl
            )
            if curl:
                return response
            if not isinstance(response, dict):
    ...
    async def replication_status(self, replicator_id: str, curl: bool = False):
        with self.__tracer.start_as_current_span(
            ...
            response = await self._send_request(
                "get", f"/_replicate/{replicator_id}", curl=curl
            )
            if curl:
                return response
            if not isinstance(response, dict):
    ...
    async def get_sub_document(
        ...
        with self.__tracer.start_as_current_span(
            ...
            keyspace = self.keyspace_builder(db_name, scope, collection)
            resp = await self._send_request("get", f"{keyspace}/{id}/{key}", curl=curl)
            if curl:
                return resp
    ...
    async def bulk_doc_op(
        ...
        with self.__tracer.start_as_current_span(
            ...
            resp = await self._send_request(
                "post", f"/{keyspace}/_bulk_docs", JSONDictionary(body), curl=curl
            )
            if curl:
                return resp
    ...
    async def blip_sync(
        ...
        with self.__tracer.start_as_current_span(
            ...
            keyspace = self.keyspace_builder(db_name, scope, collection)
            resp = await self._send_request("get", f"{keyspace}/_blipsync", curl=curl)
            if curl:
                return resp
