...
# (SyncGateway, response specs the test server serves, headers the server saw, URLs it saw)
SyncGatewayFixture = tuple[SyncGateway, list[dict], list[dict[str, str]], list[str]]
...
async def sync_gateway(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> AsyncIterator[SyncGatewayFixture]:
    ...
    every request the server saw, and `urls` the path + query string of each, so tests
    can assert on what went out on the wire."""
    ...
    async def handle(request: web.Request) -> web.Response:
        ...
        urls.append(str(request.rel_url))
    ...
    sg = SyncGateway(url=server.host, port=server.port, headers=basic_auth_headers("user", "pass"))

    yield sg, specs, received, urls
...
class TestSessionAuth:
    ...
    async def test_admin_session_sends_auth_header(self, sync_gateway: SyncGatewayFixture) -> None:
        sg, specs, received, _ = sync_gateway
    ...
    async def test_anonymous_session_sends_no_auth_header(self, sync_gateway: SyncGatewayFixture) -> None:
        ...
        async with sg._create_session(sg.secure, sg.scheme, sg.hostname, sg.port) as session:
            await sg._send_request("get", "/_status", session=session)

        assert "Authorization" not in received[0]

    @pytest.mark.asyncio
    async def test_session_sends_arbitrary_headers_and_cookies(self, sync_gateway: SyncGatewayFixture) -> None:
        """Credentials are not special: whatever headers/cookies the client was built with
        ride along on every request, so bearer tokens and SyncGatewaySession cookies work."""
        sg, specs, received, _ = sync_gateway
        specs[:] = [{"status": 200, "json": {"ok": True}}]
        ...
        async with sg._create_session(
            ...
            {"SyncGatewaySession": "abc123"},
        ) as session:
            await sg._send_request("get", "/_status", session=session)

        assert received[0].get("Authorization") == "Bearer token123"
        assert received[0].get("Cookie") == "SyncGatewaySession=abc123"


class TestGetDocument:
    @pytest.mark.asyncio
    async def test_rev_is_sent_as_a_query_param(self, sync_gateway: SyncGatewayFixture) -> None:
        sg, specs, _, urls = sync_gateway
        specs[:] = [{"status": 200, "json": {"_id": "doc1", "_rev": "1-abc", "type": "test"}}]

        doc = await sg.get_document("db1", "doc1", rev="1-abc")

        assert doc is not None
        assert doc.revision == "1-abc"
        assert doc.body == {"type": "test"}
        assert urls[0] == "/db1._default._default/doc1?rev=1-abc"

    @pytest.mark.asyncio
    async def test_no_rev_means_no_query_param(self, sync_gateway: SyncGatewayFixture) -> None:
        sg, specs, _, urls = sync_gateway
        specs[:] = [{"status": 200, "json": {"_id": "doc1", "_rev": "2-def", "type": "test"}}]

        await sg.get_document("db1", "doc1")

        assert urls[0] == "/db1._default._default/doc1"


class TestSendRequest:
    @pytest.mark.asyncio
    async def test_returns_parsed_json_on_success(self, sync_gateway: SyncGatewayFixture) -> None:
        sg, specs, _, _ = sync_gateway
        specs[:] = [{"status": 200, "json": {"ok": True}}]
...
class TestWaitForDbUp:
    ...
    @pytest.mark.asyncio
    async def test_get_user_client_does_not_touch_the_user(self, sync_gateway: SyncGatewayFixture) -> None:
        """get_user_client just points a client at the public port - no delete/add_user,
        so it is safe against users the dataset config already set up."""
        sg, specs, _, urls = sync_gateway
        specs[:] = [{"status": 200, "json": {"ok": True}}]

        async with sg.get_user_client(
            headers={"Authorization": "Bearer token123"},
            cookies={"SyncGatewaySession": "abc123"},
        ) as client:
            assert client.hostname == sg.hostname
            assert client.secure == sg.secure
            assert client.port == 4984
            assert client._headers == {"Authorization": "Bearer token123"}
            assert client._cookies == {"SyncGatewaySession": "abc123"}

        assert client._SyncGatewayBase__session.closed  # ty: ignore[unresolved-attribute]
        assert urls == [], "get_user_client should not have sent any request"
    ...
    async def test_create_user_client_context_manager(self, sync_gateway: SyncGatewayFixture) -> None:
        ...
        async with sg.create_user_client("db1", "test_user", "test_pass", ["channel1"]) as client:
            assert client.hostname == sg.hostname
            assert client.secure == sg.secure
            # Authenticated as the user, not as the admin the parent client was built with.
            assert client._headers == basic_auth_headers("test_user", "test_pass")
