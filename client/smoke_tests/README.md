...
`uv run ../../environment/aws/start_backend.py --topology ./topology.json --tdk-config-out ./config.json --tdk-config-in ./config_in.json`
...
After that it's just standard pytest stuff (e.g. `uv run pytest -v --no-header --config config.json`)
