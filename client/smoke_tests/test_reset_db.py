...
class TestResetDb:
    ...
    async def test_reset_db(self, cblpytest: CBLPyTest, name: str) -> None:
        db = await cblpytest.test_servers[0].create_and_reset_db(["test"], dataset=name)
        assert db is not None

    @pytest.mark.asyncio(loop_scope="session")
    async def test_reset_empty(self, cblpytest: CBLPyTest) -> None:
        db = await cblpytest.test_servers[0].create_and_reset_db(["test"])
        assert db is not None

    @pytest.mark.asyncio(loop_scope="session")
    async def test_reset_empty_collections(self, cblpytest: CBLPyTest) -> None:
        db = await cblpytest.test_servers[0].create_and_reset_db(["test"], collections=["a.b", "b.c", "c.d"])
        assert db is not None
