...
timeout_options = ClusterTimeoutOptions(
    kv_timeout=timedelta(seconds=5), query_timeout=timedelta(seconds=10)
)
...
def get_nodes_available_from_mobile_pool(opts):
    ...
    log_info(
        f"Getting count of node(s) with OS: {opts.nodes_os_type}, Version: {opts.nodes_os_version}"
    )
...
def reserve_nodes(opts):
    ...
    log_info(
        f"Attempting to reserve {opts.num_of_nodes} node(s) with OS: {opts.nodes_os_type}, Version: {opts.nodes_os_version}"
    )
    ...
    for row in query:
        ...
        if opts.nodes_os_type not in MOBILE_OS:
            ...
            if not vm_alive:
                query_str = (
                    "update `{}` set state=\"VM_NOT_ALIVE\" where meta().id='{}' "
            ...
            if not device_alive:
                query_str = (
                    "update `{}` set state=\"device_down\" where meta().id='{}' "
                    "and state='available'".format(doc_id, doc_id)
                )
                query = cluster.query(query_str)
...
def query_results(opts):
    ...
    query_str = (
        "select meta().id from `{}` where os='{}' "
        "AND os_version='{}' AND state='available' {}".format(
            BUCKET_NAME, opts.nodes_os_type, opts.nodes_os_version, query_phone_in_slave
        )
    )
...
def check_vm_alive(server):
    ...
    while num_retries <= SSH_NUM_RETRIES:
        try:
            ...
            print(
                "Successfully established test ssh connection to {0}. VM is recognized as valid.".format(
                    server
                )
            )
            return True
            ...
            print(
                "Exception occured while trying to establish ssh connection with {0}: {1}".format(
                    server, str(e)
                )
            )
...
def check_device_alive(device_ip):
    ...
    return (
        subprocess.run(
            args=command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        ).returncode
        == 0
    )
...
def reserve_node(doc_id, job_name, counter=0):
    ...
    try:
        ...
        if doc["state"] != "booked" and counter < 5:
            log_info(
                "Attempt to reserve node {} failed due to error {}".format(doc_id, err)
            )
...
def main():
    ...
    ** ** Reserve a device from a specific slave
    ...
    if sum(bool(opt) for opt in mutually_exclusive_options) > 1:
        raise Exception(
            "You can only specify one of --reserve-nodes, --get-available-nodes, or --release-nodes."
        )
        ...
        raise Exception(
            "You must specify at least one of --reserve-nodes, --get-available-nodes, or --release-nodes."
        )
