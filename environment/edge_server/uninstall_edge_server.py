...
def uninstall_edge_server(ip, password="couchbase"):
    ...
    result = run_remote_command(
        ip, "sudo systemctl status couchbase-edge-server", password
    )
    ...
    result = run_remote_command(
        ip, "pgrep -f /opt/couchbase-edge-server/bin/couchbase-edge-server", password
    )
    ...
    run_remote_command(
        ip, "sudo rm -rf /var/log/couchbase-edge-server /tmp/EdgeServerLog", password
    )
    ...
    if not result:
        ...
        print(
            "Couchbase Edge Server process is still present. Uninstallation may not be complete."
        )
