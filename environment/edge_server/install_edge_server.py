...
def copy_file_to_remote(
    ip, local_path, remote_path, username="root", password="couchbase"
):
...
def install_edge_server(
    ...
    if create_cert or mtls:
        run_remote_command(
            edge_server_ip, "sudo mkdir -p /opt/couchbase-edge-server/cert"
        )
        run_remote_command(
            edge_server_ip, "sudo chmod 755 -R /opt/couchbase-edge-server/cert"
        )
        run_remote_command(
            edge_server_ip,
            "sudo chown -R couchbase:couchbase /opt/couchbase-edge-server/cert",
        )
    ...
    if create_cert:
        ...
        subprocess.run(
            [
                ...
                "couchbase",
                "scp",
                f"root@{edge_server_ip}:/opt/couchbase-edge-server/cert/certfile_tls",
                "./config",
            ],
            check=True,
        )
    ...
    if database_path is not None:
        ...
        run_remote_command(
            edge_server_ip, "sudo mkdir -p /opt/couchbase-edge-server/database"
        )
        run_remote_command(
            edge_server_ip, "sudo chmod 755 -R /opt/couchbase-edge-server/database"
        )
        run_remote_command(
            edge_server_ip,
            "sudo chown -R couchbase:couchbase /opt/couchbase-edge-server/database",
        )
        ...
        subprocess.run(
            [
                ...
                "scp",
                database_path,
                f"root@{edge_server_ip}:{data_file_path}",
            ],
            check=True,
        )
        cmd_check_unzip = (
        ...
        run_remote_command(
            edge_server_ip,
            f"unzip -o {data_file_path} -d /opt/couchbase-edge-server/database",
        )

    if user:
        print(f"Creating users.json  on {edge_server_ip}...")
        run_remote_command(
            edge_server_ip, "sudo mkdir -p /opt/couchbase-edge-server/users"
        )
        run_remote_command(
            edge_server_ip, "sudo chmod 755 -R /opt/couchbase-edge-server/users"
        )
        run_remote_command(
            edge_server_ip,
            "sudo chown -R couchbase:couchbase /opt/couchbase-edge-server/users",
        )
    ...
    run_remote_command(
        edge_server_ip, f"sudo chown -R couchbase:couchbase {config_file_path}"
    )
    ...
    output = run_remote_command(
        edge_server_ip, "systemctl status couchbase-edge-server"
    )
...
def main():
    ...
    parser.add_option(
        "-e",
        "--edge-server-config",
        dest="edge_server_config",
        help="Path to the Edge server config JSON file",
        metavar="EDGE_SERVER_CONFIG",
        default=None,
    )
    parser.add_option(
        "-v",
    ...
    parser.add_option(
        ...
        dest="create_cert",
        action="store_true",
        help="Create a server certificate",
        default=False,
    )
    parser.add_option(
        "--mtls",
        dest="mtls",
        action="store_true",
        help="Create a root certificate",
        default=False,
    )
    ...
    if (
        not options.cluster_config
        or not options.edge_server_config
        or not options.version
        or not options.build
    ):
        print(
            "Usage: python install_edge_server.py -c CLUSTER_CONFIG -e EDGE_SERVER_CONFIG -v VERSION -b BUILD_NUMBER"
        )
        sys.exit(1)
