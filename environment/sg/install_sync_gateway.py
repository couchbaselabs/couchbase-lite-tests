...
def install_sync_gateway(sync_gateway_ip, sync_config, version, build):
    ...
    output, error = run_remote_command(
        sync_gateway_ip, f"wget -q {sg_package_url} -O {sg_package_file}"
    )
    ...
    output, error = run_remote_command(
        sync_gateway_ip, f"sudo dpkg -i {sg_package_file}"
    )
    ...
    run_remote_command(
        sync_gateway_ip, f"sudo chown sync_gateway:sync_gateway {config_path}"
    )
    ...
    run_remote_command(
        sync_gateway_ip, "sudo chown -R sync_gateway:sync_gateway /opt/sg/logs"
    )
...
def main():
    ...
    parser.add_option(
        ...
        "--sync-config",
        dest="sync_config",
        help="Path to the Sync Gateway config JSON file",
        metavar="SYNC_CONFIG",
        default=None,
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        "--build",
        dest="build",
        help="Sync Gateway build number",
        metavar="BUILD",
        default=None,
    )
    ...
    if (
        ...
        or not options.sync_config
        or not options.version
        or not options.build
    ):
        print(
            "Usage: python install_sync_gateway.py -c CLUSTER_CONFIG -s SYNC_CONFIG -v VERSION -b BUILD_NUMBER"
        )
        sys.exit(1)
    ...
    for sync_gateway_ip in sync_gateway_ips:
        install_sync_gateway(
            sync_gateway_ip, options.sync_config, options.version, options.build
        )
