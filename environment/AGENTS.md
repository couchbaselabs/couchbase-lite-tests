...
- `--skip-sync-gateway-start` describes the instances already listening rather than whatever `--sync-gateways` says — a run that starts nothing cannot know how many are up — reading their ports back out of the previous run's generated instance configs. Combining it with an explicit `--sync-gateways` that disagrees is a usage error. There is no `--stop-cbs` — a cluster started by `--start-cbs` is managed directly via `cbdinocluster` (reused across runs via `environment/local/.cbdinocluster-sg-cluster-id`).
