...
iterating. `--server cbs` runs against a real Couchbase Server (`--connstr`), which is slower
to set up but exercises code paths rosmar can't (bucket management, backup/restore, N1QL,
XATTRs, Couchbase Cloud REST APIs) — tests gated on `using_rosmar` are skipped or behave
differently under rosmar, so `cbs` covers more of the test suite.
...
directly, since those don't reflect a `--connstr` override:
