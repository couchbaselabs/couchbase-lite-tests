...
if [[ -n "$REQUEST_BODY" && "$REQUEST_BODY" != "{}" ]]; then
  echo "New config received. Rewriting SG host for Docker and updating config.json"
  REQUEST_BODY=$(echo "$REQUEST_BODY" | jq '
    if .replications then
      .replications |= map(
        .source |= gsub("localhost|127\\.0\\.0\\.1"; "cbl-test-sg")
        | if ((.source // "") | startswith("ws://")) then del(.pinned_cert) else . end
      )
    else . end
    | .databases |= with_entries(.value.create = (.value.create // true))
  ')
