...
def setup_server(
    ...
    Args:
        ...
        ca (CertKeyPair): The CA every Edge Server's certificate is signed by,
            shared across the whole topology so one client trust store works
            for all of them.
    """
    ...
    with open("/tmp/es_key.pem", "wb") as f:
        f.write(cert.private_pem_bytes())

    with open("/tmp/es_cert.pem", "wb") as f:
        f.write(cert.pem_bytes())

    with open("/tmp/ca_cert.pem", "wb") as f:
        f.write(ca.pem_bytes())
...
def write_client_certs() -> CertKeyPair:
    ...
    Returns:
        CertKeyPair: The CA, for signing each host's server certificate.
    """
