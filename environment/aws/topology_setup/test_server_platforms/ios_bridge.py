...
class iOSBridge(PlatformBridge):
    ...
    def stop(self, location: str) -> None:
        pid = self.__pid if self.__pid is not "" else "<error>"
    ...
    def __install_xharness(self, location: str) -> None:
        ...
        subprocess.run([str(XHARNESS_PATH), "apple", "mlaunch", "--", "--devname", location, "--installdev", self.__app_path], check=True, capture_output=False)
    ...
    def __run_xharness(self, location: str) -> None:
        self.__verify_xharness()
        result = subprocess.run([str(XHARNESS_PATH), "apple", "mlaunch", "--", "--devname", location, "--launchdev", self.__app_path], check=True, capture_output=True)
        print(result.stdout)
    ...
    def __stop_xharness(self, location: str) -> None:
        ...
        subprocess.run([str(XHARNESS_PATH), "apple", "mlaunch", "--", "--devname", location, "--killdev", self.__pid], check=True, capture_output=False)
