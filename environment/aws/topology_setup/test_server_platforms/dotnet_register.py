...
from topology_setup.test_server import TestServer, TEST_SERVER_DIR
...
class WinUIBridge(PlatformBridge):
    ...
    def install(self, location: str) -> None:
        ...
        subprocess.run(["powershell", "-NoProfile", "-Command", f"Import-Module Microsoft.PowerShell.Security; {self.__install_path} -Force"], check=True, capture_output=False, env=environ_copy)
    ...
    def run(self, location: str) -> None:
        ...
        subprocess.run(["powershell", "-NoProfile", "-Command", f"Invoke-Expression \"start shell:AppsFolder\\{self.__app_id}!App\""], check=True, capture_output=False)
        pass
    ...
    def stop(self, location: str) -> None:
        ...
        result = subprocess.run(["pwsh", "-NoProfile", "-Command", f"Get-Process -ProcessName {self.__app_name} -ErrorAction Ignore | Select-Object -Expand Id"], check=True, capture_output=True)
        if result is None:
            print("No test server process found")
            return
        
        subprocess.run(["pwsh", "-NoProfile", "-Command", f"Stop-Process {result.stdout.decode()}"], check=True, capture_output=False)
        pass
    ...
    def uninstall(self, location: str) -> None:
        ...
        subprocess.run(["powershell", "-NoProfile", "-Command", f"Get-AppxPackage {app_id_prefix}* | Remove-AppxPackage"], check=True, capture_output=False)
        pass
...
class DotnetTestServer(TestServer):
    ...
    def build(self, cbl_version: str):
        ...
        print(DOTNET_PATH)
        print(csproj_path)
        subprocess.run([DOTNET_PATH, "add", csproj_path, "package", "Couchbase.Lite.Enterprise", "--version", cbl_version], check=True, capture_output=False)
...
class DotnetTestServer_Android(DotnetTestServer):
    ...
    def create_bridge(self) -> PlatformBridge:
        return AndroidBridge(str(DOTNET_TEST_SERVER_DIR / "testserver" / "bin" / "Release" / "net8.0-android" / "com.couchbase.dotnet.testserver-Signed.apk"), "com.couchbase.dotnet.testserver")
...
class DotnetTestServer_Windows(DotnetTestServer):
    ...
    def create_bridge(self) -> PlatformBridge:
        base_path = DOTNET_TEST_SERVER_DIR / "testserver" / "bin" / "Release" / "net8.0-windows10.0.19041.0" / "win10-x64" / "AppPackages"
        install_scripts = glob.glob(str(base_path / "**" / "Install.ps1"), recursive=True)
        if not install_scripts:
            raise FileNotFoundError("Install.ps1 not found, was the test server properly built?")
        
        install_path = install_scripts[0]
        return WinUIBridge("testserver", "bf1b9964-631c-4489-91fa-a04e7f3f3765_nw4t8ysxwwgx8", install_path)
