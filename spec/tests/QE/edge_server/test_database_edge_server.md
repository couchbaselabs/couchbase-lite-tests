...
1. Configure Edge Server using an invalid DB config and assert that configuring it, or retrieving its version, raises `CblEdgeServerBadResponseError`.
...
5. Attempt a REST write to the collection and assert it raises `CblEdgeServerBadResponseError`.

The modified config is rendered to a temporary copy, so the checked-in template needs no restoring afterwards.
